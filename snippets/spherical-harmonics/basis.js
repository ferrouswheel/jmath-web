...
export function signal(coefficients, x, y, z, l = 3) {
  const b = basis(x, y, z), rgb = [0, 0, 0];
  for (let i = 0; i < (l + 1) * (l + 1); i++)
    for (let c = 0; c < 3; c++) rgb[c] += b[i] * coefficients[i][c];
  return rgb;
}
...
console.log(signal(c, 0, 0, 1, 1));  // [0.254, 0.226, 0.505] — brightest toward +z
console.log(signal(c, 0, 0, -1, 1)); // [0.254, 0.226, -0.082] — signed; clamps to black
