...
double logistic_pdf(double x, double mu, double scale) {
    if (!(isfinite(mu) && mu >= -10 && mu <= 10 && isfinite(scale) && scale >= 0.1 && scale <= 5)) return NAN;
    if (!isfinite(x)) return NAN;
    double t = exp(-fabs((x - mu) / scale));
    return t / (scale * (1 + t) * (1 + t));
}

double logistic_sample(double mu, double scale, double (*rng)(void)) {
    if (!(isfinite(mu) && mu >= -10 && mu <= 10 && isfinite(scale) && scale >= 0.1 && scale <= 5)) return NAN;
