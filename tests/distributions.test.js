...
test('known density and CDF reference values', () => {
  near(
    get('normal').density(0, { mu: 0, sigma: 1 }),
...
test('densities normalize and CDFs are monotone', () => {
  for (const d of distributions) {
    ...
    if (d.type === 'Discrete') {
      let total = 0;
      for (let k = -100; k < 1000; k++) total += d.density(k, p);
      near(total, 1);
    } else {
      const count = 20000,
        step = (hi - lo) / count;
      ...
      for (let i = 0; i < count; i++)
        total += d.density(lo + (i + 0.5) * step, p) * step;
      near(total, d.cdf(hi, p) - d.cdf(lo, p), 0.003);
    }
    let previous = 0;
    for (let i = 0; i <= 100; i++) {
      const v = d.cdf(lo + ((hi - lo) * i) / 100, p);
      assert.ok(v >= previous - 1e-12 && v <= 1);
      previous = v;
    }
  }
});
test('edge cases and parameter validation', () => {
  ...
  near(b.density(0, { n: 100, p: 0 }), 1);
  near(b.density(100, { n: 100, p: 1 }), 1);
  near(b.density(1.5, { n: 10, p: 0.5 }), 0);
  near(b.cdf(-1, { n: 10, p: 0.5 }), 0);
  ...
  assert.ok(validate(b, { n: 1.5, p: 0.5 }));
...
test('seeded samples repeat and match theoretical moments', () => {
  for (const d of distributions) {
    ...
    assert.deepEqual(
      samples.slice(0, 100),
      Array.from({ length: 100 }, () => d.sample(p, b)),
    );
    ...
    if (Number.isFinite(expected.Mean) && Number.isFinite(expected.Variance)) {
      near(
        mean,
        expected.Mean,
        Math.max(1e-9, 0.05 * Math.sqrt(expected.Variance)),
      );
      near(
        variance,
        expected.Variance,
        Math.max(1e-9, 0.05 * expected.Variance),
      );
    }
    ...
    for (let i = 1; i < 4; i++) {
      ...
      near(
        samples.filter((v) => v <= x).length / samples.length,
        d.cdf(x, p),
        0.02,
      );
    }
...
test('new distribution reference values and parameter conventions', () => {
  ...
  near(get('bernoulli').cdf(0.5, { p: 0.3 }), 0.7);
  near(get('geometric').density(2, { p: 0.5 }), 0.125);
  near(get('geometric').cdf(2, { p: 0.5 }), 0.875);
  near(get('negative_binomial').density(3, { r: 2, p: 0.5 }), 0.125);
  near(get('negative_binomial').cdf(3, { r: 2, p: 0.5 }), 0.8125);
  near(get('discrete_uniform').cdf(-2, { a: -3, b: 2 }), 1 / 3);
  near(
    get('lognormal').density(1, { mu: 0, sigma: 1 }),
    1 / Math.sqrt(2 * Math.PI),
  );
  near(get('lognormal').cdf(1, { mu: 0, sigma: 1 }), 0.5);
  near(get('laplace').density(0, { mu: 0, scale: 2 }), 0.25);
  near(get('laplace').cdf(2, { mu: 0, scale: 2 }), 1 - 0.5 / Math.E);
  near(get('logistic').density(0, { mu: 0, scale: 2 }), 0.125);
  near(get('logistic').cdf(2 * Math.log(3), { mu: 0, scale: 2 }), 0.75);
  near(get('cauchy').density(0, { center: 0, scale: 1 }), 1 / Math.PI);
  near(get('cauchy').cdf(1, { center: 0, scale: 1 }), 0.75);
  ...
  near(get('pareto').density(2, { minimum: 1, alpha: 2 }), 0.25);
  near(get('pareto').cdf(2, { minimum: 1, alpha: 2 }), 0.75);
  near(get('triangular').density(0.5, { a: 0, c: 0.5, b: 1 }), 2);
  near(get('triangular').cdf(0.25, { a: 0, c: 0.5, b: 1 }), 0.125);
  assert.equal(get('cauchy').stats({ center: 0, scale: 1 }).Mean, 'Undefined');
  assert.equal(get('pareto').stats({ minimum: 1, alpha: 1 }).Mean, '∞');
  assert.equal(get('weibull').density(0, { shape: 0.5, scale: 1 }), Infinity);
  near(get('weibull').stats({ shape: 1, scale: 2 }).Mean, 2);
  near(get('weibull').stats({ shape: 1, scale: 2 }).Variance, 4);
  assert.ok(validate(get('negative_binomial'), { r: 2.5, p: 0.5 }));
  assert.ok(validate(get('discrete_uniform'), { a: 0.5, b: 2 }));
    ...
    for (const param of d.params)
      for (const v of [param.min, param.max]) {
        ...
        for (const x of xs) {
          ...
          assert.ok(
            Number.isFinite(cdf) && cdf >= previous - 1e-12 && cdf <= 1,
            `${d.id} CDF at ${x}`,
          );
          previous = cdf;
        }
        ...
        for (let i = 0; i < 100; i++)
          assert.ok(Number.isFinite(d.sample(p, rng)), d.id);
      }
  }
});
...
test('inverse samplers handle a zero RNG draw without non-finite output', () => {
  for (const id of ['laplace', 'logistic', 'cauchy']) {
    ...
    const d = get(id),
      p = defaults(d);
    const sample = d.sample(p, () => (calls++ === 0 ? 0 : 0.5));
