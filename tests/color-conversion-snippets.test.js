...
import { colorSpaces, toLinear, fromLinear } from '../src/color-math.ts';
import {
  conversionAlgorithm,
  conversionExample,
} from '../src/color-conversion-snippets.ts';
...
const rgbs = [
  ...
  [0, 0, 1],
  [0.18, 0.18, 0.18],
  [0.043735, 0.223228, 0.921582],
  [0.9, 0.1, 0.04],
];
for (const source of colorSpaces) {
  const values = rgbs.map((rgb) =>
    fromLinear(source.id, rgb).map((v, i) =>
      Math.max(source.bounds[i][0], Math.min(source.bounds[i][1], v)),
    ),
  );
  if (source.id === 'hsl' || source.id === 'hsv') values.push([360, 100, 50]);
  if (source.id === 'oklab') values.push([0.7, 0.4, 0.3]);
  if (source.id === 'lab') values.push([50, 120, -120]);
  if (source.id === 'xyz') values.push([0, 1, 0]);
  for (const value of values)
    for (const target of colorSpaces)
      cases.push({
        source: source.id,
        target: target.id,
        values: value,
        expected: fromLinear(target.id, toLinear(source.id, value)),
      });
}
for (const language of ['js', 'python', 'c'])
  test(`${language} generic converter handles every space pair and domain cases`, () => {
    const dir = mkdtempSync(join(tmpdir(), 'jmath-convert-'));
    try {
      const algorithm = conversionAlgorithm(language);
      let output;
      if (language === 'js') {
        const context = vm.createContext({ cases });
        vm.runInContext(algorithm, context);
        output = vm.runInContext(
          'cases.map(c=>convertColor(c.values,c.source,c.target))',
          context,
        );
        for (const code of [
          'convertColor([0,0,0],"missing","lab")',
          'convertColor([NaN,0,0],"srgb","lab")',
          'convertColor([256,0,0],"srgb","lab")',
          'convertColor([0,0],"srgb","lab")',
          'convertColor([0,0,0],"srgb","constructor")',
        ])
          assert.throws(() => vm.runInContext(code, context));
      }
      if (language === 'python') {
        const file = join(dir, 'convert.py');
        writeFileSync(
          file,
          algorithm +
      ...
      if (language === 'c') {
        const file = join(dir, 'convert.c'),
          binary = join(dir, 'convert');
        writeFileSync(
          file,
          algorithm +
        ...
        output = execFileSync(binary, [], {
          input: cases
            .map((c) =>
              [ids.indexOf(c.source), ids.indexOf(c.target), ...c.values].join(
                ' ',
              ),
            )
            .join('\n'),
      ...
      cases.forEach((c, i) => {
        if (c.expected === null) assert.equal(output[i], null);
        else c.expected.forEach((v, j) => close(output[i][j], v));
      });
    } finally {
...
test('usage examples supply source, target and coordinates separately', () => {
  for (const language of ['js', 'python', 'c']) {
    const first = conversionExample(language, {
      source: 'srgb',
      target: 'lab',
      values: [59, 130, 246],
    });
    const next = conversionExample(language, {
      source: 'hsl',
      target: 'oklab',
      values: [180, 50, 50],
    });
    assert.notEqual(first, next);
    assert.ok(first.includes(language === 'c' ? 'SRGB' : 'srgb'));
    assert.ok(next.includes(language === 'c' ? 'OKLAB' : 'oklab'));
    assert.throws(() =>
      conversionExample(language, {
        source: 'srgb',
        target: 'xyz',
        values: [NaN, 0, 0],
      }),
