...
import {
  ...
  polygonDots,
  isPrime,
} from '../src/sequences.ts';
import { sequenceGeometry } from '../src/sequence-visuals.ts';
...
const fixtures = {
  ...
  pentagonal: [0, 1, 5, 12, 22, 35, 51],
  hexagonal: [0, 1, 6, 15, 28, 45, 66],
...
test('sequence definitions agree with known terms and large exact values', () => {
  ...
  for (const s of sequences)
    fixtures[s.id].forEach((v, i) =>
      assert.equal(sequenceTerm(s, s.min + i), BigInt(v), s.id),
    );
  ...
  assert.equal(
    sequenceTerm(get('powers-of-two'), 100),
    1267650600228229401496703205376n,
  );
  assert.equal(sequenceTerm(get('primes'), 1000), 7919n);
  assert.equal(sequenceTerm(get('cubes'), 1000), 1000000000n);
  for (const s of sequences)
    for (const n of [-1, 1001, 1.5, NaN, Infinity])
      assert.throws(() => sequenceTerm(s, n));
  assert.throws(() => sequenceTerm(get('primes'), 0));
});
test('figurate identities hold across the supported range', () => {
  for (let n = 1; n <= 1000; n++) {
    assert.equal(
      sequenceTerm(get('squares'), n),
      sequenceTerm(get('triangular'), n) +
        sequenceTerm(get('triangular'), n - 1),
    );
    assert.equal(
      sequenceTerm(get('pentagonal'), n),
      BigInt(n) + 3n * sequenceTerm(get('triangular'), n - 1),
    );
    if (2 * n - 1 <= 1000)
      assert.equal(
        sequenceTerm(get('hexagonal'), n),
        sequenceTerm(get('triangular'), 2 * n - 1),
      );
  }
});
test('polygon dot counts and added layers agree with formulas', () => {
  for (const [id, sides] of [
    ...
    for (let n = 0; n <= s.visualMax; n++) {
      ...
      if (n)
        assert.equal(
          BigInt(points.filter((p) => p.new).length),
          sequenceTerm(s, n) - sequenceTerm(s, n - 1),
          `${id} added layer at ${n}`,
        );
    }
  }
...
test('Fibonacci tiles form a non-overlapping rectangle with the stated area', () => {
  for (let n = 1; n <= 9; n++) {
    ...
    const area = tiles.reduce((s, t) => s + BigInt(t.size) ** 2n, 0n);
    assert.equal(area, fibonacci(n) * fibonacci(n + 1));
...
test('geometry is finite, capped explicitly, and represents the correct objects', () => {
  for (const s of sequences)
    for (const n of [s.min, s.initial, s.visualMax, 1000]) {
      const g = sequenceGeometry(s, n);
      assert.equal(g.index, Math.min(n, s.visualMax));
      assert.ok(!/NaN|Infinity/.test(g.svg));
      assert.ok(g.caption.length > 20);
    }
  for (let n = 1; n <= 6; n++) {
    assert.equal(
      (sequenceGeometry(get('cubes'), n).svg.match(/data-unit-cube/g) || [])
        .length,
      n ** 3,
    );
    assert.equal(
      (sequenceGeometry(get('powers-of-two'), n).svg.match(/<circle /g) || [])
        .length,
      2 ** (n + 1) - 1,
    );
  }
